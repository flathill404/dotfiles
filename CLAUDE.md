...
The setup script runs: Xcode CLI tools → Homebrew → Brew Bundle → Prepare directories → Stow configs → Proto languages → VSCode extensions → fzf keybindings → default shell → GPG agent → Fix permissions → macOS hardening.
